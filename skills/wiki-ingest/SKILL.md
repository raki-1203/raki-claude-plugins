...
updated: {today}
description: "{한 줄 — 무엇·왜·결론이 다 들어가게}"
comment: "{사용자가 제공했으면 기록. 없으면 생략}"
---
```

> **`description` 은 짧게가 아니라 판단 가능하게 쓴다.** 이 한 줄이 그대로 `index.md` 항목이 되고, 읽는 쪽은 **항목만 보고 페이지를 열지 말지 정한다.** 그래서 주제 이름이 아니라 결론까지 담아야 한다 — 무엇에 대한 것인지, 왜 저장했는지, 무엇이 결론인지. 길이 상한은 두지 않는다(실측 중앙값 65자). 분류 라벨로 끝나면 실패다.
>
> - ✅ `"Karpathy 3-Layer를 스킬 진화에 적용해 SOTA 상회 — 스킬은 롤백, 위키는 영속"`
> - ❌ `"에이전트 스킬 진화 논문"` — 열어봐야 아는 라벨
