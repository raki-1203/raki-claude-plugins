...
> **상세 절차**: `references/notebooklm-guide.md`의 "대용량 파일 처리" 섹션 참조.

| 크기 | 대응 |
|------|------|
| < 2MB | 그대로 업로드 |
| 2~10MB | `--compress` 재시도 → 여전히 > 2MB면 파일 구분선 기준 분할 업로드 |
| > 10MB | 바로 분할 업로드 |

**절대 하지 않을 것**: 업로드 실패 시 원본(더 큰 파일)을 그대로 재업로드. 반드시 분할 후 여러 소스로 업로드.
