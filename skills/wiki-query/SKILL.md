...
**판정형 시그널 (답변형의 하위 케이스 — `verdicts.md` 를 먼저 본다):**
- 과거 판단을 묻는다: "왜 뺐어", "왜 안 써", "왜 제거", "이미 검토했나", "전에 봤던"
- 현재 채택 여부를 묻는다: "쓰고 있어?", "도입했어?", "지금 뭐 써?"
- 새 도구의 도입을 묻는다: "X 도입할까?", "X 써도 돼?", "X 어때?"

도구·라이브러리·플러그인·접근법의 **이름이 등장하면서 판단을 묻는** 질문이면 판정형으로 본다. 개념 설명 요청("X가 뭐야?")은 판정형이 아니다.

분기:
- **답변형** → Step 1 (overview.md → index.md 읽기)로 진행
- **판정형** → Step 1 의 0단계부터 (verdicts.md → 나머지 순서 동일)
...
0. **판정형이면 `verdicts.md` 먼저 읽기**: `$VAULT/verdicts.md` 는 도구·접근의 채택/기각/제거 판정 색인이다. 표 전체를 읽고 질문의 대상이 이미 판정됐는지 확인한다.
   - **판정이 있으면** 그 행의 `판정`·`한 줄 근거`·`날짜` 를 답변 첫머리에 두고, `상세` 링크의 페이지를 읽어 근거를 보강한다. 답변은 "이미 판정됨"에서 시작해야 한다 — 사용자가 모르고 재검토를 시작하는 것을 막는 게 이 파일의 목적이다.
   - **`기각` 과 `제거` 를 구분해 전한다**: `기각` 은 써보지 않고 내린 판단이라 새 근거가 나오면 뒤집힐 여지가 크고, `제거` 는 써보고 내린 판단이라 그렇지 않다. 재검토 가치가 다르므로 뭉뚱그리지 않는다.
   - **판정이 없으면** "판정 이력 없음"을 명시하고 일반 답변형으로 계속 진행한다. 없는 것을 있는 것처럼 말하지 않는다.
   - `updated:` 가 오래됐거나 판정 날짜가 6개월 이상 지났으면 "판단 시점이 오래됨"을 함께 알린다.
