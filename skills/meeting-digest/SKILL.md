...
산출물 (성공 시):
- `$RAW_DIR/transcript.speakers.txt` — `[화자1] … [화자2] …` 블록
- `$RAW_DIR/transcript.speakers.json` — 세그먼트별 speaker 라벨 + 타임스탬프
...
**화자 분리 결과 우선 사용**: `transcript.speakers.txt`가 있으면 그것을 읽어라 (`[화자1]`/`[화자2]` 라벨 포함). 이 경우 "주요 논의"에서 누가 무슨 입장이었는지를 **추측이 아닌 실제 화자 라벨**로 쓴다. 단 화자1/화자2는 익명 라벨이므로, `--attendees`나 문맥(자기소개·호칭)으로 실제 이름과 매칭되면 이름으로 치환하고, 불확실하면 "화자1" 그대로 둔다. speakers 파일이 없으면 기존대로 `transcript.txt`로 진행(화자는 문맥 추정).
...
3. **프로젝트 페이지 링크 추가** (`wiki/projects/$PROJECT.md` 존재 시):
   ...
   - 있으면 그 섹션 맨 위에 다음 한 줄 추가:
     ...
     ```
4. **회의록 색인 허브 갱신** (`wiki/projects/$PROJECT/회의록.md` **존재 시에만**):
   - `## 회의록 목록 (시간순)` 섹션에 **시간순 제자리**로 한 줄 삽입한다. 프로젝트 페이지의 `## Meetings` 는 최신이 맨 위지만 **이 허브는 오름차순**이다 — 맨 위에 넣지 말 것:
     ```
     - [[<date>-<slug>]] — <한 줄 요약: 회의록 frontmatter description 을 압축. 제목 반복 금지>
     ```
     링크는 `meetings/<project>/` 경로가 아니라 **파일명만** 쓴다(기존 항목과 동일 형식).
   - frontmatter `description` 에 건수가 적혀 있으면(`"… 회의록 N건 시간순 색인 허브"`) **N을 갱신**한다. 이 숫자는 방치되면 바로 틀어진다.
   - **파일이 없으면 만들지 않는다** — 허브를 두는 프로젝트만 두면 된다(lint 단계에서 처리).
   - `## 회의록 목록` 섹션이 없으면 삽입하지 말고 경고만 출력한다.

   > ⚠️ **macOS 파일명은 NFD, 문서 안 한글은 NFC** 다. `os.listdir` 로 얻은 슬러그와 문서에서 파싱한 링크를 그대로 비교하면 **한글이 든 항목만 조용히 불일치**한다(ASCII 항목은 매칭돼 더 헷갈린다). 대조·중복 검사 전에 양쪽을 `unicodedata.normalize('NFC', …)` 로 맞출 것. 2026-09-22 국민카드 허브 백필에서 실측.

5. `log.md`에 한 줄 추가 (파일 끝):
   ```
   ## [YYYY-MM-DD] <project> | 회의록: <title>
   ```
6. `index.md` 의 회의록 섹션이 있으면 카운트 갱신. 없으면 생성하지 않음 (lint 단계에서 처리).

> **왜 두 곳을 쓰는가** — 회의록 링크는 `wiki/projects/{project}.md` 의 `## Meetings` 와 `wiki/projects/{project}/회의록.md` 허브 **두 곳**에 존재한다. 후자는 2026-07-12 lint 에서 회의록이 고아로 잡히던 문제를 풀려고 생긴 색인이다. 이 스킬이 전자만 갱신하던 탓에 국민카드 허브는 **2026-07-08 이후 갱신이 끊겨 28건이 빠졌고 그 사실을 두 달 넘게 아무도 몰랐다**(2026-09-22 발견·백필). 한쪽만 쓰면 반드시 뒤처진다.
...
원본 (raw):
  raw/meetings/<project>/<date>-<slug>/
    ...
    ├ transcript.json
    ├ transcript.srt
    └ transcript.speakers.txt  (<N>명 화자 / 화자 분리 시에만)

회의록 (wiki):
  wiki/meetings/<project>/<date>-<slug>.md

프로젝트 페이지 업데이트: <wiki/projects/<project>.md 에 링크 추가 / 페이지 없음>
회의록 색인 허브:      <wiki/projects/<project>/회의록.md 에 링크 추가 (총 N건) / 허브 없음>
```
...
| 화자 분리 미설정/실패 | 건너뛰고 화자 라벨 없이 회의록 생성 (중단 안 함) |
| slug 빈 결과 | `--title` 명시 요청 |
| 중복 wiki 파일 | overwrite/rename/cancel 질문 |
| 색인 허브 없음 / `## 회의록 목록` 섹션 없음 | 건너뛰고 경고 한 줄 (중단 안 함, 허브를 새로 만들지 않음) |
...
⚠️ **회의 1건으로 결론 내지 말 것.** 2026-08-08에 회의 1건(3구간)으로 내린 결론 2개가
2026-08-10 재현 검증(회의 2건 5구간)에서 뒤집혔다 — `--logprob-threshold -0.5`가 "필수"라는
것과 "Qwen 1.7B는 `--context` 유출 없음"이 둘 다 재현되지 않았다. 3건 이상에서 같은 방향이
나올 때만 코드 주석·위키에 단정을 박는다.
