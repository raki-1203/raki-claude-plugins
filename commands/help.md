...
## 용도
회의 녹음 파일(mp3/m4a/wav/mp4 등)을 받아 mlx-whisper로 한국어 전사한 뒤
...
옵션 (생략 시 인터랙티브 질문/자동 추정):
  ...
  --model large-v3           Whisper 모델 (기본: large-v3)
  --attendees "이름1,이름2"  참석자 frontmatter

## 동작
1. OBSIDIAN_VAULT_PATH + 의존성 검증 (mlx-whisper, ffmpeg)
2. 원본 오디오를 raw/meetings/{project}/{date}-{slug}/audio.{ext}에 복사
3. mlx-whisper로 전사 → transcript.txt/.json/.srt
...
## 요구사항
`mlx-whisper`, `ffmpeg` (없으면 `/rakis:setup` 실행)
