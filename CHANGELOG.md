...
- `source-fetch`: **본문 속 참고 링크도 독립 소스로 수집**(Phase 2.5 신설, `references/link-follow.md`). SNS 게시물·뉴스레터는 대개 자기가 알맹이가 아니라 알맹이를 가리키는데, 기존에는 "무엇을 소개했다"는 사실만 raw에 남고 소개된 대상은 안 들어왔다 — 2026-09-23 `linkedin-soulai-awesome-jev` 실측: 게시물 4.4KB 중 실질 내용은 `github.com/AnotiaWang/awesome-jev` 한 줄이었고, 게시물이 주장한 가격("입력 100만 토큰당 4.2센트, 출력 무료")을 대조할 원본이 raw에 없었다. **링크를 따라간 뒤 확인하니 레포 90KB 전체에 그 주장의 근거가 없었다.** 가드 셋: `article` 타입만(repo·paper는 링크가 수백 개 — `awesome-jev`에 돌리면 인덱싱된 488개가 전부 대상), 깊이 1 고정(순환 탐지 불필요), 부모당 수집 8개 상한(`fanout-limit`). 분류는 "읽을거리 하나를 가리키는 링크만" — 구독처(`t.me`·채널홈·프로필)·루트 랜딩·플랫폼 내부·정적 에셋 제외하되 **애매하면 수집**한다(잘못 수집한 raw는 폴더 하나 지우면 끝이지만 잘못 제외한 링크는 아무도 눈치채지 못한다). 제외한 링크도 사유와 함께 `meta.json`의 `references`에 전부 남기고, 자식에는 `referenced_by`로 역링크. 자식은 독립 slug·독립 `meta.json`을 가지므로 `wiki-ingest` 수정 없이 전수 스캔에 그대로 잡힌다. `--no-follow` 로 기록만 하고 수집은 끌 수 있다. 함께 기록한 함정: **`lnkd.in/p/{code}` 는 `curl -I -L` 에 `Location` 이 오지만 `lnkd.in/{code}` 는 안 온다** — 인터스티셜 HTML을 GET해 대상 URL을 긁어야 하고, 모르면 링크 해석 단계에서 그냥 실패한다.
...
- `test.sh`: index.md 일관성 검사가 **리다이렉트 stub 을 미등록으로 오탐**하던 문제. 같은 문서가 두 슬러그로 중복 수집되면 내용을 한쪽으로 병합하고 남은 쪽은 stub 으로 남긴 뒤 index.md 에서 내린다 — 파일을 지우면 `raw/` 의 `meta.json` 이 그대로라 다음 `wiki-ingest` 가 페이지를 되살리기 때문이다(불변 계층). 즉 미등록이 **의도된 상태**인데 테스트가 이를 몰라 실패시켰고, 2026-09-19 `awesome-vibe-invest` 병합 이후 **pre-push 훅이 이 레포의 모든 push 를 막고 있었다**. 본문의 폐기 마커(`> **이 페이지는 폐기됐다.`)가 있는 페이지는 검사에서 제외하고 제외 건수를 출력한다.
- `meeting-digest`: Phase 5 가 `wiki/projects/{project}.md` 의 `## Meetings` 만 갱신하고 **`wiki/projects/{project}/회의록.md` 색인 허브는 건드리지 않던 문제**. 같은 링크를 두 곳에서 유지하는 구조인데 한쪽만 자동이라 다른 쪽이 조용히 썩는다 — 국민카드 허브는 **2026-07-08 이후 갱신이 끊겨 회의록 50건 중 28건이 빠진 채 두 달 넘게 방치**됐고(데일리미팅 11건·스프린트 리뷰 5건·보안성심의·지식저장소 등), 수동으로 두 건만 들어가 있어 겉보기에는 살아 있는 색인으로 보였다. Phase 5 에 허브 갱신 단계를 신설했다 — **존재할 때만** 갱신(없으면 만들지 않음), 프로젝트 페이지와 달리 **시간순 오름차순 제자리 삽입**, frontmatter `description` 의 건수 표기도 함께 갱신. Phase 6 출력과 에러 표에도 반영. 함께 기록한 함정: **macOS 파일명은 NFD·문서 안 한글은 NFC** 라 `os.listdir` 결과와 문서에서 파싱한 링크를 그대로 비교하면 한글 항목만 불일치하고 ASCII 항목은 매칭돼 오진하기 쉽다. 대조 전 `unicodedata.normalize('NFC', …)` 필수.
