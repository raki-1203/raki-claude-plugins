...
- `source-fetch`: **Phase 2.5 참조 링크 수집** 신설. 수집한 원본이 참조하는 자료를 같이 모은다 — 단 **1차 자료만 자동**이고 판정은 도메인 화이트리스트로만 한다(arXiv·OpenReview·ACL·PDF·GitHub·HuggingFace·원본이 직접 링크한 공식 프로젝트 페이지). 블로그·SNS·뉴스·랜딩 페이지는 수집하지 않고 후보로만 보고한다. 재귀 없이 1단계, 기존 `meta.json` 있으면 건너뜀, 수집물 `meta.json` 에 `referenced_by` 기록. 계기: LinkedIn의 NoRA 논문 요약 하나를 검증하려고 논문 PDF·공식 페이지·구현 저장소를 뒤늦게 전부 모아야 했다(2026-09-18). 2차 요약만 위키에 남으면 사실 확인이 원저자 요약에 묶인다. 화이트리스트 밖은 2차로 간주하는 보수적 기본값 — 자동 수집은 되돌리기가 비싸다(21 MB repomix가 동기화 vault로 들어온다).
